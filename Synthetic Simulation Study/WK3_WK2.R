...
Sims3to2 <- function(flow, time,R,C,Z_0, cycles2, sdnoise2, ind2, model){
  ...
  ggplot(data=df_R2, aes(R)) + 
    ...
    ggsave(stringr::str_c(model,"histogramR3to2.png"))
  ...
  ggplot(data=df_C2, aes(C)) + 
    ...
    theme(panel.background = element_rect(fill = NA, color = "black"))+
    ggsave(stringr::str_c(model,"histogramC3to2.png"))
  ...
  optimal_param_P <- pressurecalculator_wk2(flow,time,R_real,C_real)
  true_P <- pressurecalculator_wk3(flow,time,R,C,Z_0)
  
  #mean bias 
  b_mean <- apply(abs(b_hat),2,sum)/100
  b_sort <- sort(abs(b_hat))
  
  sort(SAVE_field)
  SAVE_field
  apply(abs(SAVE_field),1,sort)
  ...
  df_optim <- data.frame(x = time, y1 = optimal_P$P, y2  = true_P$P, lb = lower, ub = upper, y3 = optimal_param_P$P)
  ...
  ggplot(df_field, aes(x = Time(s), y = AorticPressure(mmHg)))+
    ...
    geom_line(aes(x=x, y=y3,color = "Real Parameter P"), data = df_optim)+
    geom_line(aes(x=x, y=y2,color = "Real P"), data = df_optim)+
    geom_line(data = df_optim, aes(x = x, y = lb), color = "#33CC66", size = 1, linetype = "dashed") + 
    geom_line(data = df_optim, aes(x = x, y = ub), color = "#33CC66", size = 1, linetype = "dashed")+
    ...
    theme(panel.background = element_rect(fill = NA, color = "black"))+
    ggsave(stringr::str_c(model,"fit3to2.png"))
...
#test values
#pressurecalculator_wk2(flow*0.7,time, R,C)
#pressurecalculator_wk3(flow*0.7,time,R,C,Z)

#run
HP <- Sims3to2(flow,time,R,C,Z_HP,cycles2,sdnoise2,ind2,"HP")
non_HP <- Sims3to2(flow,time,R,C,Z_nonHP,cycles2,sdnoise2,ind2, "non_HP")
